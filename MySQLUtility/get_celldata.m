...
if ~use_cellid
	...
	sqlstr=['select c.cellid, ts, wave, c.sessid, single from cells k, spktimes as c, sessions as s '...
		    ' where s.sessid=c.sessid and k.cellid=c.cellid and s.ratname="{S}" and experimenter="{S}" and '...
		   ' sessiondate in (' datestr(2:end) ') order by sessiondate'];
	
	[S.cellid, S.ts, S.wave, S.sessid, S.single]=bdata(sqlstr,ratname, experimenter);
else
	...
	[S.cellid, S.ts, S.wave, S.sessid, S.single]=bdata(['select s.cellid, ts, wave, s.sessid, single from spktimes s, cells c where s.cellid=c.cellid and s.cellid in (' cellstr(2:end) ') order by sessid']);
