...
if Settings('compare','GENERAL','use_timers',2),
...
%       Settings('load') call, so that we can look at the RIGS;fake_rp_box
...
skip_globals = Settings('compare','COMPATIBILITY', ...
...
[dbstop_setting errID] = Settings('get','GENERAL','dbstop_if');
if errID,
...
[Main_Code_Directory errID errmsg] = ...
    Settings('get','GENERAL','Main_Code_Directory');
if errID, return; end;
...
[Main_Data_Directory errID errmsg] = ...
    Settings('get','GENERAL','Main_Data_Directory');
if errID, return; end;
...
[steMS errID errmsg] = ...
    Settings('get','RIGS','state_machine_server');
if errID, return; end;
[sndMS errID errmsg] = ...
    Settings('get','RIGS','sound_machine_server');
if errID, return; end;
...
global fake_rp_box;
[fake_rp_box errID errmsg] = Settings('get','RIGS','fake_rp_box');
if errID, return; end;

global state_machine_server;
[state_machine_server errID errmsg] = ...
    Settings('get','RIGS','state_machine_server'); if errID, return; end;

global sound_machine_server;
[sound_machine_server errID errmsg] = ...
    Settings('get','RIGS','sound_machine_server'); if errID, return; end;

global cvsroot_string;
[cvsroot_string errID errmsg] = Settings('get','CVS','CVSROOT_STRING');
if errID, return; end;
...
[outputs errID_i errmsg_i] = Settings('get','DIOLINES','all');
if errID_i,
    errID = 1;
    errmsg = [errorlocation ': Attempt to retrieve DIOLINES settings group failed. Settings(''get'',''DIOLINES'',''all'') returned the following error (ID: ' int2str(errID_i) '): ' errmsg_i ];
    return;
end;
...
softsound_play_sounds = Settings('get','EMULATOR','softsound_play_sounds');

global pump_ontime;
[pump_ontime errID errmsg]      = Settings('get','PUMPS','pump_ontime');
if errID, return; end;

global pump_offtime;
[pump_offtime errID errmsg]     = Settings('get','PUMPS','pump_offtime');
if errID, return; end;

global sound_sample_rate;
[sound_sample_rate errID errmsg] = Settings('get','SOUND','sound_sample_rate');
if errID, return; end;

global Solo_Try_Catch_Flag;
[Solo_Try_Catch_Flag errID] = Settings('get','GENERAL','Solo_Try_Catch_Flag');
...
global Solo_datadir;
[Solo_datadir errID errmsg] = Settings('get','GENERAL','Main_Data_Directory');
if errID, return; end;
