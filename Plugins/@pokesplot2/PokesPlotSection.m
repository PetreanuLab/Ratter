...
try
    ...
    switch action
        ...
        case 'init'
            ...
            elseif exist('STATE_COLORS', 'var') && isa(STATE_COLORS, 'SoloParamHandle')
            ...
            elseif exist('WAVE_COLORS', 'var') && isa(WAVE_COLORS, 'SoloParamHandle')
            ...
            if ~exist('VISIBLE_STATES_LIST', 'var') || ~isa(VISIBLE_STATES_LIST, 'SoloParamHandle')
                ...
                set_saveable(VISIBLE_STATES_LIST, true);
                set_save_with_settings(VISIBLE_STATES_LIST, true);
            end
            if ~exist('VISIBLE_POKES_LIST', 'var') || ~isa(VISIBLE_POKES_LIST, 'SoloParamHandle')
                ...
                set_saveable(VISIBLE_POKES_LIST, true);
                set_save_with_settings(VISIBLE_POKES_LIST, true);
            end
            if ~exist('VISIBLE_WAVES_LIST', 'var') || ~isa(VISIBLE_WAVES_LIST, 'SoloParamHandle')
                ...
                set_saveable(VISIBLE_WAVES_LIST, true);
                set_save_with_settings(VISIBLE_WAVES_LIST, true);
            end
        ...
        case {'axpokesplot_callback', 'plotclick'}
            ...
            [enable_script_hooks errID1] = Settings('compare', 'AUX_SCRIPT', 'Disable_Aux_Scripts',0);
            [enable_ppclick_hook errID2] = Settings('compare', 'AUX_SCRIPT', 'Enable_On_PPClick_Script', 1);
            [script errID3] = Settings('get', 'AUX_SCRIPT', 'On_PPClick_Script');
            [args errID4] = Settings('get', 'AUX_SCRIPT', 'On_PPClick_Args');
            ...
            elseif isa(obj, 'neurobrowser')
                ...
                savedate_v = datestr(savedate_v, 'yymmdd');
                fix_date_format=0;
            else
            ...
            evalstring = [	script				...	%     filename of script
                ' '			args				...	%     args from settings file
                ' '			experimenter_v		...	%     name of experimenter
                ' '			ratname_v			...	%     name of rat
                ' '			savedate_v			...	%     date data was saved
                ' '			class(obj)			...	%     protocol name
                ' '         int2str(trialNum)	...	%     clicked trial
                ' '         int2str(timeExp)	...	%     selected time minus start time of experiment
                ' '         int2str(timeTri)	...	%     selected time minus start time of clicked trial
                ' '         int2str(timeExpEoT)	];	%     selected trial's end time
