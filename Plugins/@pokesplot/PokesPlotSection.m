...
try
    ...
    switch action,
        ...
        case 'update',
            ...
            if value(start_trial) <0 % in this mode update to allows have start_trials on the screen
                set_ylimits(n_started_trials, value(axpokesplot), value(trial_limits), ...
                   max(0,n_done_trials-abs(value(start_trial))), n_done_trials+1, value(ntrials));
            else
                set_ylimits(n_started_trials, value(axpokesplot), value(trial_limits), ...
       ...
       case 'trial_limits'
        ...
        case 'set_time_axis'
            ...
            t = varargin{1};
            set(value(axpokesplot), 'XLim', [value(t(1)) value(t(2))]);
            % BA PROBLEM AM NOT UPDATAING GIU .. not not sure how to do
            % this... fix later
        case 'time_axis'
            set(value(axpokesplot), 'XLim', [value(t0) value(t1)]);
        ...
        case 'plotclick'
            ...
            if ~strcmp(value(plotclick_action),'runs external script'), return; end;
            ...
            [enable_script_hooks errID1] = ...
                Settings('compare','AUX_SCRIPT','Disable_Aux_Scripts',0);
            [enable_ppclick_hook errID2] = ...
                Settings('compare','AUX_SCRIPT','Enable_On_PPClick_Script',1);
            [script errID3] = ...
                Settings('get','AUX_SCRIPT','On_PPClick_Script');
            [args errID4] = ...
                Settings('get','AUX_SCRIPT','On_PPClick_Args');
            ...
            elseif isa(obj,'neurobrowser')
                ...
                savedate_v=savedate_v(3:end);
                % The save date should be in YYMMDD format.  MySQL is YYYY-MM-DD
                % format.  So i get rid of the - and then chop off the first 2
                % digits of the YYYY
                fix_date_format=0;
            else
