function [handles xpos] = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    xpos = []; % SP added - x coords for bar centers
	for i = 1:numbars
