function output = averageshape(s1,s2)

if size(s1,1) == 2 %Shape defined by coordinates
    m1 = poly2mask(s1(2,:),s1(1,:),ceil(max(max(s1)))+10,ceil(max(max(s1)))+10);
else
    m1 = s1;
end
...
for i = 0.01:0.01:1
    ...
    output(c,:) = [mean([tp1new(v1,1) tp2new(v2,1)]) mean([tp1new(v1,2) tp2new(v2,2)])];
end
