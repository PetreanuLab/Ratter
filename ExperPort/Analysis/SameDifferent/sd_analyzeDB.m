% [a]= sd_analyzeDB(ratstr, daterange, {'tau' 30}, {'fignum_in', []}, {'daily' 0})
%
% Runs a trial-type-by-trial-type hit_history analysis on a rat in the
% SameDifferent protocol. Brings up a
...
function [aa]= sd_analyzeDB(ratstr, daterange, varargin)
...
for rat_i = 1:numel(all_rats)
    ...
    if ~daily,
        for i=1:ntypes,
          ...
          us{i} = setdiff(us{i}, find(isnan(hit_history))); % ignore violation (NaN) trials
          if ~isempty(us{i}),
          ...
          guys = hit_history(us{i}); 
          newguys = zeros(size(guys));
