<<<<<<< ratave.m
%My call:
%ratave({'A020'}, -100, 'varnames', {'SoundInterface_LeftSoundVol$', SettingsSection_pro_prob}, 'vals', [0.02, 0])
=======
%ProRats:
%ratave({'A021', 'A023','A024','A025', 'A028', 'A029', 'A030', 'A033'}, -100,   'varnames', {'SoundInterface_LeftSoundVol$'}, 'vals', [0.02])
%AntiRats:
%ratave({'A020', 'A022','A026','A027', 'A031', 'A032'}, -100, 'varnames', {'SoundInterface_LeftSoundVol$'}, 'vals', [0.02])
>>>>>>> 1.5
...
for r = 1:nrats,
    ...
    if ~usedb,
<<<<<<< ratave.m
        if ~isempty(experimenters{r}),
            exp = experimenters{r};
        else
            error('Experimenters must be specified for ~useDB);
        end;
=======
        if ~isempty(experimenters{r})
                exp = experimenters{r};
...
set(fignum, 'Name', ['Rat Average, n = ' num2str(nrats - nanrats)]);
