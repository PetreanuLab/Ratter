...
bRandomizeConditions = 1; % BA Set this to zero to do in blocks and in 1 to do randomized blocks
...
switch action,
    case 'init'
        ...
        if bRandomizeConditions
            SoloParamHandle(obj, 'BlockMatrix', 'value', randperm(value(LaserBlock)));
        else
            SoloParamHandle(obj, 'BlockMatrix', 'value', [1:value(LaserBlock)]);
        end
    ...
    case 'reset_laser_block'
        if bRandomizeConditions
            ...
            BlockMatrix.value = [1:value(LaserBlock)];
        end
        Counter.value = 1;
