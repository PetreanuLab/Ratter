...
%             TrialsPerBlockSet.value = NaN;
            ...
            sma = add_state(sma, 'name', 'waiting_for_both_pokes',...
                'input_to_statechange', {'Lin', 'left_poke_in', 'Rin', 'right_poke_in'});

            % Deliver water left
            sma = add_state(sma, 'name', 'left_poke_in', 'output_actions', {'DOut', left1water}, ...
                'self_timer', value(lValve), 'input_to_statechange', {'Tup', 'final_state', 'Lout', 'final_state'});    

            % Deliver water right
            sma = add_state(sma, 'name', 'right_poke_in', 'output_actions', {'DOut', right1water}, ...
...
%             TrialsPerBlockSet.value = 50;
            ...
            sma = add_state(sma, 'name', 'premature_cout', 'self_timer', 0.1, ...
                'input_to_statechange', {'Tup', 'final_state'});
            
            % Waiting for right or left poke
            sma = add_state(sma, 'name', 'waiting_for_both_pokes', 'self_timer', value(timeToGetReward),...
                'input_to_statechange', {'Lin', 'left_poke_in', 'Rin', 'right_poke_in', 'Tup', 'too_late'});
            
            % Right poke in
            sma = add_state(sma, 'name', 'right_poke_in', 'output_actions', {'DOut', right1water},...
                'self_timer', value(rValve), 'input_to_statechange', {'Tup', 'final_state', 'Lout', 'final_state'});
            ...
            % Too late
            sma = add_state(sma, 'name', 'too_late', 'self_timer', 0.1, ...
                'input_to_statechange', {'Tup', 'final_state'});
...
%                 disp(value(OlfBank))
                disp(value(valveNumber))
                  disp(value(odorPresent))
                  strcmpi(value(odorPresent), 'Yes')
%-------------------------------------------------------------------------%
...
%-------------------------------------------------------------------------%
            ...
            % Center poke in
            sma = add_state(sma, 'name', 'center_poke_in', 'self_timer', (value(waitingTimeOlf)), ...
            ...
            sma = add_state(sma, 'name', 'cin_odor', 'output_actions', {value(OlfBank), value(valveNumber)},...
            ...
            if strcmpi(value(odorPresent), 'No') == 1
                % Waiting for right or left poke
                sma = add_state(sma, 'name', 'waiting_for_both_pokes', 'self_timer', value(timeToGetReward),...
                    'input_to_statechange', {'Lin', 'lin_water_olf', 'Rin', 'rin_no_water_olf', 'Tup', 'too_late'});
                ...
                % Left poke in - water
                sma = add_state(sma, 'name', 'lin_water_olf', 'output_actions', {'DOut', left1water},...
                    'self_timer', value(lValve), 'input_to_statechange', {'Tup', 'final_state', 'Lout', 'final_state'});
                
                % Right poke in - no water
                sma = add_state(sma, 'name', 'rin_no_water_olf',...
            ...
            elseif strcmpi(value(odorPresent), 'Yes') == 1
                ...
                % Waiting for right or left poke
                sma = add_state(sma, 'name', 'waiting_for_both_pokes', 'self_timer', value(timeToGetReward),...
                    'input_to_statechange', {'Rin', 'rin_water_olf', 'Lin', 'lin_no_water_olf', 'Tup', 'too_late'});
                ...
                % Right poke in - water
                sma = add_state(sma, 'name', 'rin_water_olf', 'output_actions', {'DOut', right1water}, ...
                    'self_timer', value(rValve), 'input_to_statechange', {'Tup', 'final_state', 'Rout', 'final_state'});
                
                % Left poke in - no water
                sma = add_state(sma, 'name', 'lin_no_water_olf',...
...
%     case 'update',
%         
%         if (strcmp (latest_parsed_events.states.ending_state, 'left_poke_in_water') == 1 || ...
%                 strcmp (latest_parsed_events.states.ending_state, 'right_poke_in_water') == 1)
%             a = 1
%         elseif (strcmp (latest_parsed_events.states.ending_state, 'premature_cout') == 1 || ...
%                 strcmp (latest_parsed_events.states.ending_state, 'too_late') == 1)
%             a = 0
%         end
