...
function [x, y] = ChoiceSection_Olf(obj, action, x, y)
...
%     x = 1; y = 1;
    ...
    DispParam(obj, 'error', 0, x, y, ...
        'label', 'Error', 'labelfraction', 0.5); next_row(y, 1);
    DispParam(obj, 'correct', 0, x, y, ...
...
%     end;
    ...
    if (~isempty(parsed_events.states.lin_no_water_olf) == 1) || ...
            (~isempty(parsed_events.states.lin_water_olf) == 1),
        leftPort.value = value(leftPort) + 1
    elseif (~isempty(parsed_events.states.rin_no_water_olf) == 1) || ...
            (~isempty(parsed_events.states.rin_water_olf) == 1),
        rightPort.value = value(rightPort) + 1
    end
    
    if (~isempty(parsed_events.states.lin_no_water_olf) == 1) || ...
            (~isempty(parsed_events.states.rin_no_water_olf) == 1),
        error.value = value(leftPort) + 1
    elseif (~isempty(parsed_events.states.lin_water_olf) == 1) || ...
            (~isempty(parsed_events.states.rin_water_olf) == 1),
        correct.value = value(rightPort) + 1
    end
