...
%          "Custom" files are loaded on the fly with Settings('load').)
%
...
%
%
function varargout = Settings(action, varargin)
...
switch action,
    ...
    case 'get',
        %     [settingval errID errmsg]  = Settings('get', <setting_group_name>, <setting_name>)
        ...
        if isempty(BControl_Settings) || NoSettingsLoaded(BControl_Settings),
            [errID_internal errmsg_internal] = Settings('load');
            if errID_internal,
                errID = 8;
    ...
    case 'compare',
        ...
        %       Settings('compare', <setting_group_name>, <setting_name>, <comparison_val>)
        %     Generate an error iff* we have n args where n~=3.
    ...
    case 'load',
        %     [errID errmsg] = Settings('load', <name_of_settings_file>)
        ...
        if nargin>1,
            for i = 1:length(varargin),
                ...
                if isempty(filenames{i}) || ~ischar(filenames{i}),
                    errID = 10;
                    errmsg = [errorlocation ' : LOAD called incorrectly. Requires no additional arguments OR any number of additional nonempty filename strings. Sample calls: Settings(''load'') or Settings(''load'',''mr_settings_file.conf'', ''customsettings.conf'',''randomsettings.conf'')'];
                    varargout = {errID errmsg};
                    %err_and_errdlg(errmsg);
                    return;
                end;
