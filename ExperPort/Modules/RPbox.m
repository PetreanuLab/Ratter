...
% What is the above hack variable? As currently configured through RPBox.m,
% all parts of a 'state35' call must be finished before
% ReadyToStartTrial(statemachine) is called. (In RPBox.m, ReadyToStartTrial
% is represented by a call to invokeWrapper(RP, 'SoftTrg', 10) at the end
% of rpbox('state35').) But that means that there might be a bunch of
% things being done *after* the next trial's state matrix has already been
% sent and the trial is ready to go, yet the trial doesn't start yet. If
% you send the state matrix with 4 arguments, rpbox('send_matrix', mtrx,
% arg3, arg4), where arg3 is the regular 3rd argument (see
% @RTLSM/SetStateMatrix.m for an explanation of that 3rd arg), then, when
% arg4 ~= 0, ReadyToStartTrial will be called immediately after sending the
% matrix and the behavior box will start the new trial immediately. This
% hack_variable will be set to 1, and the next ReadyToStartTrial call (from
% rpbox('state35')'s call to invokeWrapper(RP, 'SoftTrg', 10), will be
% ignored. When the call to invokeWrapper(Rp, 'SoftTrg', 10) is completed,
% the hack variable is reset to 0, no matter what its value before that
% call (see invokeWrapper.m).
%
% BOTTOM LINE FOR A USER: If you want to reduce your in-between-trials dead
% time, don't send your state matrix with rpbox('send_matrix', mtrx, arg3),
% but send it with rpbox('send_matrix', mtrx, arg3, 1). The rest will be
% taken care of for you. (arg3 should be whatever you were using before).
%   If you are one of those users that sends their state matrix using the
% @RTLSM object directly, through its SendStateMatrix.m method, then sorry:
% can't help you. Switch to dispatcher.
%   --Carlos.
%
...
%             Event       =   invokeWrapper(RP,'ReadTagVex','Event',LastEventCounter,EventCounter-LastEventCounter,'F32','F64',1);
            ...
            % DirectEvents bypasses invokewrapper and gets the events
            % directly from the state machine-- this means it is not
            % backwards compatible with the old state machines under RP boxes by Tucker Davis Technologies.
            DirectEventsTmp = GetEvents(rpbox('getstatemachine'), LastEventCounter/2+1, EventCounter/2);
            % The State Machine returns an event with an indication of both
            % which sate it started from and which it went to; in the old
            % machines we indicated this with a fake event, input channel
            % 0, at the state that we went to. Let's replicate this here
            % for backwards compatibility with software:
            DirectEvents = zeros(size(DirectEventsTmp,1)*2, size(DirectEventsTmp,2)); % make room for fake events
            DirectEvents(1:2:end,:) = DirectEventsTmp;   % First event in a pair indicates state we started from
            DirectEvents(2:2:end,:) = DirectEventsTmp;   % Next one is a copy, except its input channel is 0 and
            % its state number is the state we went *to*:
            DirectEvents(2:2:end,2) = 0; DirectEvents(2:2:end,1) = DirectEventsTmp(:,4); % ok, we're done
            
            % Input columns sometimes include SchedWaves, so it isn't
            % always 1:7. so we do this (the extra one is for Tup):
            nInputCols = length(GetInputEvents(rpbox('getstatemachine')))+1;
...
%                     evnt_t(event_n)=EventTime(i);
                    chan_chk = find(bitget(DirectEvents(i,2), 1:nInputCols));
                    state(event_n)  = DirectEvents(i,1);
                    evnt_t(event_n) = DirectEvents(i,3);
                    if state(event_n) == 35  
...
%                             evnt_t(event_n)=EventTime(i);
                ...
                SetParam(me,'Event','value',num2str([state(end),chan(end)]),'user',[state',chan',evnt_t']);
...
%         SaveParamsTrial(me);
        ...
        % put in code from invokeWrapper.m -- includes hack var which
        % ignores 'ready-to-start' signal
    ...
    case 'send_matrix'
        ...
        elseif nargin==4
          if varargin{4}~=0,
            send_matrix(varargin{2}, varargin{3});
            machine = rpbox('getstatemachine');
            if isa(machine, 'softsm'), machine = SetPCReadyFlag(machine);
            else                       machine = ReadyToStartTrial(machine);
            end;
            rpbox('setstatemachine', machine);
            private_hack_ignore_next_ready_to_start_trial = 1;
            % See comments at top of RPBox.m for what the above hack variable is
          else
            send_matrix(varargin{2}, varargin{3});
          end;
...
if isempty(machine)
    ...
    machine = SetInputEvents(machine, 6, 'ai');
    output_routing = { struct('type', 'dout', ...
        'data', '0-30') ; ...
        struct('type', 'sound', ...
        'data', sprintf('%d',0)) };
    machine = SetOutputRouting(machine, output_routing);
    machine = SetJumpstate(machine,35);
    machine = rpbox('setstatemachine', machine);
else
