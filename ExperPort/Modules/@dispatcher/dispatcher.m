...
switch action,
  ...
  case {'set_protocol'},     ProtocolsSection(obj, action, varargin{1});
  ...
  case 'init_bypass',
      ...
      [unsorted_channels errID] = Settings('get','DIOLINES', 'all');
      if errID,
          error(['In Dispatcher, request for DIOLINES group has failed with Settings(''get'',''DIOLINES'',''all'') returning error ID ' int2str(errID) ' and error message: ' errmsg]);
      end;
