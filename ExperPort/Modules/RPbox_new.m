...
elseif invoke(RPS,'ConnectRM1','USB',2)     %connect to RM1 using 'USB' method
    ...
    if ~invoke(RPS,'LoadCOF',[GetParam('rpbox','protocol_path') '\3StereoSoundRM1.rco']);
...
for i=1:length(id)  
    ...
    if id(i)==1,
        ...
        datain_chk = invoke(RPS,'WriteTagV', 'datain1a', 0, beep{1}(1,:)); %when use with regular speaker

        datain_chk = invoke(RPS,'WriteTagV', 'datain1b', 0, beep{1}(2,:)); %when use with regular speaker

    else

        datain_chk=invoke(RPS,'WriteTagV',['datain' num2str(id(i))],0,beep{id(i)}); %when use with regular speaker
