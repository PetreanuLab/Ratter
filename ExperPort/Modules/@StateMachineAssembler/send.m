...
function [stm, assembled_state_names, sm] = send(sma, sm, varargin)
   ...
   if ~isempty(setdiff({'S','P', 'L', 'R'}, fieldnames(input_lines))),
       error(['Sorry, send.m only support input_lines structures with exactly three fields,\n' ...
           '''S'', ''P'', ''L'', and ''R''. Bug Carlos to extend it to what you need or do it yourself.\n']);
   end;
   ...
   for i=1:length(sma.sched_waves),
       if ~sma.sched_waves(i).is_ao % skip if AOut sched waves
   ...
   for i=1:length(sma.sched_waves),
      if sma.sched_waves(i).is_ao % only for AOut sched waves
          sm = SetScheduledWaves(sm, sma.sched_waves(i).id, sma.sched_waves(i).AOut, sma.sched_waves(i).loop, sma.sched_waves(i).two_by_n_matrix);
      end;
   end;
